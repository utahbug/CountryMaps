...
export function acceptsIslandDrop(point,target,viewport,neighbors=[],radius=26){
 if(!inBox(point,viewport)||Math.hypot(point.x-target.x,point.y-target.y)>radius)return false;
 // Ocean tolerance must never turn a drop on another country's land into a success.
 return !neighbors.some(n=>n.contains(point));
}
...
// The wrapper is at least a comfortable pointer size while the geographic path
// inside it stays at the map's actual rendered scale.
export function dragPreviewGeometry(country,x,y,scale,minTarget=44){
 const visible=draggedBounds(country,x,y,scale),frameW=Math.max(minTarget,visible.w+8),frameH=Math.max(minTarget,visible.h+8);
 return {
  visible,
  frame:{x:x-frameW/2,y:y-frameH/2,w:frameW,h:frameH},
  viewBox:[country.anchor[0]-frameW/(2*scale),country.anchor[1]-frameH/(2*scale),frameW/scale,frameH/scale]
 };
}
