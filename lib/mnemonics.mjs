...
export class MnemonicPlayer{
 ...
 clear(){clearTimeout(this.timer);this.animations.forEach(a=>a.cancel());this.animations=[];this.node?.remove();this.host?.classList.remove('mnemonic-playing');this.host=null;this.node=null;this.active=null;}
 ...
 play(id,data,host){
  ...
  const node=document.createElement('span');node.className='mnemonic-overlay';node.dataset.mnemonic=c.id;node.setAttribute('aria-hidden','true');
  ...
  node.querySelector('p').textContent=c.caption;host.append(node);host.classList.add('mnemonic-playing');this.host=host;this.node=node;
  if(!reduced){
