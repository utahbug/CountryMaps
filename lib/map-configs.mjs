...
const plannedArea=(id,name,submaps=[],overlapping=false)=>({
 id,name,status:'planned',showOnHome:true,preview:'assets/maps/'+id+'-planned.svg',
 unitType:'country',terminology:countryTerms,manifest:{expectedCount:null,units:[]},
 coverage:{kind:overlapping?'overlapping-learning-region':'continent',exclusive:false,submaps},
 activities:Object.fromEntries(['explorer','reveal','puzzle'].map(mode=>[mode,{enabled:false,status:'planned'}])),
});
// Canonical manifests: the Americas expose only map views; Africa retains all activities.
export const mapDefinitions={
 europe:plannedArea('europe','Europe',['europe-regions']),
 asia:plannedArea('asia','Asia',['western-asia','central-asia','south-asia','east-asia','southeast-asia']),
 'middle-east':plannedArea('middle-east','Middle East',[],true),
 'united-states':{
 ...
 africa:{referenceUrl:new URL('../data/africa-learning.json',import.meta.url),eyebrow:'COUNTRIES OF THE WORLD',id:'africa',name:'Africa',status:'ready',unitType:'country',terminology:countryTerms,
