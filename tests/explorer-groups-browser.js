 ...
 try{
  ...
  for(const group of groupedCountries(data.countries)){
   ...
   for(const c of group.countries){
    click('[data-list-country="'+c.id+'"]');assert(q('.map-readout strong').textContent===c.name,'select '+c.id);assert(q('[data-country="'+c.id+'"]').classList.contains('selected'),'highlight '+c.id);assert(signature()===fitted,'selection keeps geographic context '+c.id);
